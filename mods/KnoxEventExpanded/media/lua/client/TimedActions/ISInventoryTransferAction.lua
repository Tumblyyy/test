...
function ISInventoryTransferAction:isValid()
    ...
    if self.destContainer == "npc" or self.srcContainer == "npc" and not NpcInventoryUI.mockContainerOpen then
        return false;
    end
	self.dontAdd = false;
...
--	print("perform on item", self.item, self.item:getDisplayName())
	...
	for i,item in ipairs(queuedItem.items) do
        ...
        if self.destContainer:getType()=="npc" then
			self:removeItemOnCharacter();
			if self.item:IsWeapon() then
            	giveNpcItem(NpcInventoryUI.npcObj:getOnlineID(), self.item:getFullType(), self.item:getCondition(), self.item:isContainsClip(), self.item:getCurrentAmmoCount(), self.item:isRoundChambered());
			elseif self.item:IsClothing() then
				giveNpcItem(NpcInventoryUI.npcObj:getOnlineID(), self.item:getFullType(), self.item:getCondition());
			else
				giveNpcItem(NpcInventoryUI.npcObj:getOnlineID(), self.item:getFullType());
			end
        ...
        elseif self.srcContainer:getType()=="npc" then
            takeNpcItem(NpcInventoryUI.npcObj:getOnlineID(), tostring(NpcInventoryUI.mockContainerLookup[self.item:getID()]));
            self.srcContainer:DoRemoveItem(self.item);
            self.destContainer:AddItem(self.item);
        else
