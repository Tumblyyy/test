...
function ISWorldMap:onScavengeNpc(worldX, worldY)
    local scavengeRegion = ScavengeRegion.new(worldX, worldY, 40);
    local i = 0;
    for _, npc in ipairs(selectedGroupNps) do
        npc:setGoal("RoamingSubTree");
        npc:setSubGoal("ScavengeSubTree");
        local tree = npc:getBehaviorTree();
        local goalNode = tree:getGoalNode();
        local roamingNode = goalNode:getChildWithName("RoamingSubTree");
        local scavengeNode = roamingNode:getChildWithName("ScavengeSubTree");
        scavengeNode:setData("ScavengeSubTreeScavengeRegion", scavengeRegion);
        scavengeNode:setData("LootingSubTreeCategoryAmmo", lootAmmo);
        scavengeNode:setData("LootingSubTreeCategoryFood", lootFood);
        scavengeNode:setData("LootingSubTreeCategoryFirstAid", lootFirstAid);
        scavengeNode:setData("LootingSubTreeCategoryMats", lootMaterials);
        scavengeNode:setData("LootingSubTreeCategoryGuns", lootGuns);
        scavengeNode:setData("LootingSubTreeCategoryTools", lootTools);
        scavengeNode:setData("LootingSubTreeCategoryWater", lootWater);
        scavengeNode:setData("LootingSubTreeCategoryWeapons", lootWeapons);
        print("NPC " .. npc:getUsername() .. " told to scavenge");
        if i == 0 then
            npc:Say("Scavenge now? If you say so...");
        end
        i = i + i;
    end
...
function ISWorldMap:addSafehouse(worldX,  worldY)
    local textureSymbol = self.symbolsAPI:addTexture("FaceDead", worldX, worldY)
...
function WorldMapNpcOverlayUI:onMouseUp(x, y)
    print("Got coords " .. tostring(x) .. ", " .. tostring(y));
    local npcIndex = math.floor(x/NPCUI_ELEMENT_WIDTH);
    local player = getPlayer();
    local group = player:getGroup();
    if group == nil then
        return;
