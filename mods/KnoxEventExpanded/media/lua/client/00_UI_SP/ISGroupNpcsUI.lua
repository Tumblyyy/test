...
function ISGroupNpcsUI:populateList()
    ...
    for i=0, npcList:size()-1 do
        local npcData = npcList:get(i);
        local panelEntry = ISPanel:new(self.width / 20, y, 9 * self.width / 10, itemHeight);
        x = 0;
        local nameButton = ISButton:new(x, 0, nameWidth, itemHeight, npcData:getUsername(), nil, function() end);
        x = x + nameWidth + paddingX;
        local taskButton = ISButton:new(x, 0, taskWidth, itemHeight, npcData:getGoalPretty(), nil, function() end);
        x = x + taskWidth + paddingX;
        local subTaskButton = ISButton:new(x, 0, subTaskWidth, itemHeight, npcData:getSubGoalPretty(), nil, function() end);
        x = x + subTaskWidth + paddingX;
        local relationButton = ISButton:new(x, 0, relationWidth, itemHeight, npcData:getPlayerRelation(player), nil, function() end);
        x = x + relationWidth + paddingX;
        local removeButton = ISButton:new(x, 0, removeWidth, itemHeight, getText("IGUI_KnoxEvent_NpcOverview_Remove_Button"), nil, function() askNpcToLeaveGroup(nil, npcData); end);
    ...
    if safeHouse ~= nil then
        local releaseSafeHouseButton = ISButton:new(panelWidth / 100, 8 * self.height / 10, panelWidth / 10, itemHeight, getText("ContextMenu_SafehouseRelease"), nil, function()
            releasePlayerSafehouse();
            player:Say(getText("IGUI_KnoxEvent_Player_Speech_SafehouseRelease"));
        end);
        self:addChild(releaseSafeHouseButton);
    end
