...
function NpcOverlayUI:populateList(data)
    table.wipe(self.npcList);
    self.npcListLen = 0;
    for _, value in pairs(data) do
        local id = value["id"]
        local name = value["name"]
        local task = value["task"]
        local subtask = value["subtask"]
        local newNpc = {};
        newNpc.id = id
        newNpc.name = name;
        newNpc.task = task;
        newNpc.subtask = subtask;
        table.insert(self.npcList, self.npcListLen, newNpc)
        self.npcListLen = self.npcListLen + 1;
    end
end
...
function NpcOverlayUI:onMouseUp(x, y)
    ...
    local npc_index = math.floor(y/NPCUI_ELEMENT_HEIGHT);
    local playerData = getPlayerData(0);
    if playerData.npcOverlayUI.npcList[npc_index] ~= nil then
        local npc_id = tonumber(playerData.npcOverlayUI.npcList[npc_index]["id"]);
...
local function createStayCursor(npcId)
    local bo = ISNpcStayCursor:new("", "", npcId, getPlayer());
...
function NpcOverlayUI:onRightMouseUp(x, y)
    ...
    local npc_index = math.floor(y/NPCUI_ELEMENT_HEIGHT);
    if playerData.npcOverlayUI.npcList[npc_index] ~= nil then
        local npc_id = playerData.npcOverlayUI.npcList[npc_index]["id"];
        task_context_menu:addOption(getText("ContextMenu_KnoxEvent_Barricade"), nil, function() tellNpcBarricade(nil, npc_id) end);
        task_context_menu:addOption(getText("ContextMenu_KnoxEvent_ChopTrees"), nil, function() tellNpcChopTrees(nil, npc_id) end);
        task_context_menu:addOption(getText("ContextMenu_KnoxEvent_DumpCorpses"), nil, function() tellNpcDumpCorpses(nil, npc_id) end);
        task_context_menu:addOption(getText("ContextMenu_KnoxEvent_DumpLoot"), nil, function() tellNpcDumpLoot(nil, npc_id) end);
        task_context_menu:addOption(getText("ContextMenu_KnoxEvent_Follow"), nil, function() tellNpcFollow(nil, npc_id) end);
        task_context_menu:addOption(getText("ContextMenu_KnoxEvent_Stay"), nil, function() createStayCursor(npc_id) end);
        task_context_menu:addOption(getText("ContextMenu_KnoxEvent_Guard"), nil, function() tellNpcGuard(nil, npc_id) end);
        task_context_menu:addOption(getText("ContextMenu_KnoxEvent_Rest"), nil, function() tellNpcRest(nil, npc_id) end);
        ...
        print("No npc at index " .. tostring(npc_index));
    end
end
...
function NpcLootOverlayUI:prerender()
    ...
    if lootAmmo then
        self:drawTextZoomed("Ammo", 3, 0 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("Ammo", 3, 0 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    end
    ...
    if lootFirstAid then
        self:drawTextZoomed("First Aid", 3, 1 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("First Aid", 3, 1 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    end
    ...
    if lootFood then
        self:drawTextZoomed("Food", 3, 2 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("Food", 3, 2 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    end
    self:drawRectBorderStatic(0, 2 * NPCLOOTUI_ELEMENT_HEIGHT, NPCLOOTUI_ELEMENT_WIDTH, NPCLOOTUI_ELEMENT_HEIGHT, self.borderColor.a, self.borderColor.r, self.borderColor.g, self.borderColor.b);

    if lootGuns then
        self:drawTextZoomed("Guns", 3, 3 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("Guns", 3, 3 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    end
    ...
    if lootMaterials then
        self:drawTextZoomed("Materials", 3, 4 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("Materials", 3, 4 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    end
    ...
    if lootTools then
        self:drawTextZoomed("Tools", 3, 5 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("Tools", 3, 5 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    end
    ...
    if lootWater then
        self:drawTextZoomed("Water", 3, 6 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("Water", 3, 6 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    end
    ...
    if lootWeapons then
        self:drawTextZoomed("Weapons", 3, 7 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("Weapons", 3, 7 * NPCLOOTUI_ELEMENT_HEIGHT + 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    end
...
function NpcAggressiveOverlayUI:prerender()
    if isGroupAggressive then
        self:drawTextZoomed("NPCs stance: Hostile", 3, 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("NPCs stance: Neutral", 3, 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    end
...
function NpcRetreatOverlayUI:prerender()
    if isGroupRetreating then
        self:drawTextZoomed("Retreat", 3, 5, 1.0, 0.8, 0.1, 0.1, FONT_HGT_SMALL);
    else
        self:drawTextZoomed("Fight", 3, 5, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
    end
    self:drawRectBorderStatic(0, 0, NPCLOOTUI_ELEMENT_WIDTH * 2, NPCLOOTUI_ELEMENT_HEIGHT, self.borderColor.a, self.borderColor.r, self.borderColor.g, self.borderColor.b);
end
...
function NpcDialogueUI:onMouseUp(x, y)
    ...
    local npcObj = getPlayerByOnlineID(self.npcId);
    if self.submenu == 0 then
        ...
        elseif y <= optionsTwo and y >= DIALOGUE_FIRST_OFFSET then
            askRecentToNpc(self.npcId);
        elseif y <= optionsThree and y >= DIALOGUE_FIRST_OFFSET and npcObj and not isNpcInGroup(npcObj) then
            askNpcToJoinGroup(self.npcId);
        elseif y <= optionsFour and y >= DIALOGUE_FIRST_OFFSET then
            stopTalkingToNpc(self.npcId);
    ...
    elseif self.submenu == 1 then
        if y <= optionOne and y >= DIALOGUE_FIRST_OFFSET then
            smallTalkWeather(self.npcId);
        elseif y <= optionsTwo and y >= DIALOGUE_FIRST_OFFSET then
            smallTalkAge(self.npcId);
        elseif y <= optionsThree and y >= DIALOGUE_FIRST_OFFSET then
            smallTalkOccupation(self.npcId);
        elseif y <= optionsFour and y >= DIALOGUE_FIRST_OFFSET then
            self.submenu = 0;
        end
    end
...
function NpcDialogueUI:prerender()
    if self.npcName ~= nil then
        ...
        if self.submenu == 0 then
            ...
            y = y + DIALOGUE_ROW_HEIGHT;
            local npcObj = getPlayerByOnlineID(self.npcId);
            if npcObj and not isNpcInGroup(npcObj) then
            ...
            y = y + DIALOGUE_ROW_HEIGHT;
            self:drawTextZoomed("Leave", 3, y + DIALOGUE_PADDING, 1.0, 1.0, 1.0, 1.0, FONT_HGT_SMALL);
            self:drawRectBorderStatic(0, y, self.width, self.height - y, self.borderColor.a, self.borderColor.r, self.borderColor.g, self.borderColor.b);
            self:drawRectBorderStatic(0, 0, self.width, self.height, self.borderColor.a, self.borderColor.r, self.borderColor.g, self.borderColor.b);
        elseif self.submenu == 1 then
            y = y + DIALOGUE_FIRST_OFFSET;
