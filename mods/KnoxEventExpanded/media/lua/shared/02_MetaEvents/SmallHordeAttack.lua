...
SmallHordeAttack = ISBaseObject:derive("SmallHordeAttack");

function SmallHordeAttack:setup()
    local safehouse = self.group:getSafehouse();
    local x = safehouse:getX() + safehouse:getW()/2;
    local y = safehouse:getY() + safehouse:getH()/2;
    sendNpcRadioMessageWithRange(x, y, 90000, 200, "To anyone listening, we are under attack by zombies at " .. tostring(x) .. "," .. tostring(y) .. ". Please help us!");
    --sendSafehouseMarker(x, y, 200);
end

function SmallHordeAttack:simulate()
...
function SmallHordeAttack:loaded()
    local safehouse = self.group:getSafehouse();
...
function SmallHordeAttack:finish()
    local groupList = self.group:getGroupMembers();
...
function SmallHordeAttack:new(group)
    local o = {}
    setmetatable(o, self)
    self.__index = self
    o.group = group;
    o.eventObj = LuaNpcMetaSafehouseEvent.new(group, o);
    o.nbrOfZombies = ZombRand(2, 3);
    return o
end